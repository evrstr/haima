...
<script id="goodsTpl" type="text/html">
    <a href="<<# if(d.in_stock > 0){ >>/buy/<< d.id >><<#  } else { >>javascript:void(0);<<# }; >>" class="goods-box" style="<<# if(d.in_stock <= 0){ >>cursor:not-allowed;<<# }; >>">
        ...
        <div class="goods-price">
            ￥<< d.actual_price>>
                <<# layui.each(d.wholesale_price_arr, function(index, item){>>
                    <div>{{ __('luna.goods_disc_1') }}
                        << item[0]>>{{ __('luna.goods_disc_2') }}
                            << item[1]>>{{ __('luna.goods_disc_3') }}
                    </div>
                    <<# });>>
        </div>
        <div class="goods-num">
            <div>
                <p style="width: << d.proportion >>%;"></p>
            </div>
            <span>{{ __('luna.goods_surplus') }}
                << d.in_stock>>{{ __('luna.goods_unit') }}
            </span>
        </div>
    </a>
</script>
