...
class CouponsController extends AdminController
{
    ...
    {
        $pd_class = $request->get('q');
        return Products::where('pd_class', $pd_class)->get(['id', DB::raw('pd_name as text')]);


        // $namelist =  Products::where('pd_class', $pd_class)->get(['id', 'pd_name']);
        // $nameClass = [];
        // foreach ($namelist as $val) {
        //     //$nameClass[$val['id']] = array("id" => $val->id, "text" => $val->pd_name);
        //     array_push($nameClass, array("id" => $val->id, "text" => $val->pd_name));
        // }
        // return $nameClass;
        //return response()->json($nameClass);
    }
